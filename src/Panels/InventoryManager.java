...
public class InventoryManager {
	...
	public void swapItems (int positionInv, int positionEquip) {	
			...
			if (positionEquip == 3) {
				if (!getItemEq(5).getName().equals("")) {
					if (getFirstEmptySlot() == -1) {
						MainApplet.addInfo("You don't have enough space to equip this weapon.");
						return;
					}
				}
			}
		...
		if (positionEquip == 5) {
			if (itemsEq[3].isTwoHanded) {
				MainApplet.addInfo("You have a two-handed weapon equipped");
				return;
			}
		}
		...
		MainApplet.getGamePanel().equipScreen().repaint();
		MainApplet.getGamePanel().invScreen().repaint();
		
		Connector.Send("equis~"+positionInv+"~"+positionEquip+"#");
	}
	public void ejectItem (int positionEquip) {
		int positionInv = MainApplet.actPlayer.getInventoryManager().getFirstEmptySlot();
		if (positionInv == -1) {
			MainApplet.addInfo("You have no space in your inventory.");
			return;
		}
	...
	public void addItemsTr (int positionInv) {
		if (getFirstEmptySlotTr() < 0) {
			MainApplet.addInfo("You can't trade more than 12 items at once.");
			return;
		}
	...
	public void ejectItemTr (int positionTr) {
		...
		MainApplet.getGamePanel().tradeScreen().repaint();
		MainApplet.getGamePanel().invScreen().repaint();
	}
	...
	public void setItemTr (Item item, int position) {	
		if (!item.isTradeable()) {
			MainApplet.addInfo("This item is not tradeable.");
			return;
		}
