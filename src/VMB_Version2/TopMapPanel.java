...
public class TopMapPanel extends JPanel {
	...
	private void drawFences() {
		...
		//corners
		map.Fields[x1+mx][y1+my] = new MapFields.FenceNW(0,0);
		map.Fields[x1+mx][y2+my] = new MapFields.FenceSW(0,0);
		map.Fields[x2+mx][y1+my] = new MapFields.FenceNE(0,0);
		map.Fields[x2+mx][y2+my] = new MapFields.FenceSE(0,0);
		//north+south side:
		for (int i=1;i<w-1;i++) {
			map.Fields[x1+i+mx][y1+my] = new MapFields.FenceNorth(0,0);
			map.Fields[x1+i+mx][y2+my] = new MapFields.FenceSouth(0,0);
		}
		//east+west side:
		for (int i=1;i<h-1;i++) {
			map.Fields[x1+mx][y1+i+my] = new MapFields.FenceWest(0,0);
			map.Fields[x2+mx][y1+i+my] = new MapFields.FenceEast(0,0);
		}
		fieldsChanged = true;
		map.repaint();
	}
