...
public class ExternalPlayer {
	...
	public ExternalPlayer (String name, int x, int y, int mapLV) {
		...
		this.Img100 = Util.getImage("player/niels.png");
	    if (this.name.equals("umer"))
	    	 this.Img100 = Util.getImage("player/Eye_of_the_Sword.png");
	    
	    this.Img30 = Util.getImage("player/moving/person_vorne.gif");
	}
	...
	public static void setPosition(String name, String x, String y, String z) {		
		...
		//debug
		App.getGamePanel().optScreen().refresh();
	}
		...
		for (int i=0;i<extPlNum;i++) {
			try {
				...
				if (extPlayer[i].mapLV == map.getMapLV()) {
					g.drawImage(temp.Img30,(temp.mapX-map.getMapX()-1)*30,(temp.mapY-map.getMapY()-1)*30,App.app);
