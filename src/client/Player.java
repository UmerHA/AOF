...
import client.panels.InventoryManager;
...
public class Player {
	...
	public boolean setIdentitiy (String username, String password) {
		...
		fileName = Util.DATA_DIR + "UserData/"+ username + ".txt";
		File userFile = new File(fileName);
		
		if ((!userFile.exists()) || (!userFile.isFile())) {
			App.app.alert("Player doesn't exist");
			return false;
		}
	    ...
	    this.pic = Util.getImage("pics/player/niels.gif");
	    if (this.name.equals("umer"))
	    	 this.pic = Util.getImage("pics/player/Eye_of_the_Sword.gif");
	    ...
	    App.getGamePanel().invScreen().setManager(inv);
	    App.getGamePanel().equipScreen().setManager(inv);
	    App.getGamePanel().infoScreen().getNameLabel().setText(username);
	    App.map.setLV(Byte.parseByte(data[10]));
	    App.map.setCurrentField(mapX,mapY);
	...
	public void setData(String x, String y, String mapLV) {
		...
		if (Byte.parseByte(mapLV) != App.map.getMapLV())//only reload map, when mapLV changed
			App.map.setLV(Byte.parseByte(mapLV));
	...
	public void recalcPosXY () {
		...
		App.map.changeMapCoords(difX, difY);
	}
	
	public void logOut () {
		App.getGamePanel().chatbox().output().discardAllInfo();
		App.app.setContentPanel(0);
	}


	public void move (short dir) {
		//System.out.println("Player.move :: moving... ; x = "+posX+" ; y = "+posY);
		if (App.getGamePanel().chatbox().inConversation())
			App.CVManager().stopConversation();
		...
		Connector.Send("mve~"+tempX+"~"+tempY+"~"+App.map.getMapLV()+"#");
	}
	...
	private void addToHP (short difference) {
		if ((difference > 0)&&(mom_hp == max_hp)) {
			mom_hp = max_hp;
			App.addInfo("You already have full HP");
			return;
		}
		mom_hp += difference;
		if (mom_hp <= 0)
		...
		App.getGamePanel().infoScreen().refresh();
		//MainApplet.map.repaint();		
	}
	...
	private void die () {
		...
		short i = 0;
		while (i < 3) {
			int position = App.getRandom(0, 29);
			if (this.inv.destroyItem(position))
				i++;
		}
		
		
		App.map.changeMapCoords(0, 0);
		App.addInfo("You died and lost 3 items...");
		App.app.alert("You died");
	}
	...
	public void paint (Graphics g) {
		...
		Image temp;
		String tempS = "player/moving/";
		
		//System.out.println ("pointing direction : " + pointingDir);
		
		switch (pointingDir) {
			case 1  : temp = Util.getImage(tempS+"person_links.gif");break;
			case 2  : temp = Util.getImage(tempS+"person_oben.gif");break;
			case 3  : temp = Util.getImage(tempS+"person_rechts.gif");break;
			case 4  : temp = Util.getImage(tempS+"person_unten.gif");break; 
			default : temp = Util.getImage(tempS+"person_vorne.gif");break;	 	// default
		}
		
		//g.drawImage(temp, x+stepX, y+stepY, MainApplet.applet);
		g.drawImage(temp, x, y, App.app);
	...
	public void refreshFriendsList () {
		client.connection.Connector.Send("frlg~#");
	}
	public void updateFriendsList (String[] friends) {//called by server
		App.getGamePanel().chatScreen().setFriendData(friends);
	}
	...
	public void addItem (Item item) {
		...
		if (!itemAdded)
			App.addInfo("You don't have enough space to pick up another "+item.getName()+".");
	}
