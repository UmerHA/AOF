...
public class Joe extends TNPC {
	...
	public void use() {
		...
		if (App.actPlayer.cocoMilk == 0) {
			talk0();
		} else if (App.actPlayer.cocoMilk == 1){
			talk11();
		} else {
			talk21();
		}
	...
	private void talk4() {
		CVM.say("At least my mother didn't hate me enough to name me "+App.actPlayer.getName()+".");
	...
	public void talk17() {
		int tempInt = App.actPlayer.getInventoryManager().searchItem("Coconut");
		if (tempInt == -1) {
		...
		App.addInfo("You show Joe the coconut.");
		CVM.say("Thanks a lot buddy.");
		CVM.say("Here, have some money");
		App.addInfo("Joe takes the coconut and gives you 500 coins");
		App.actPlayer.getInventoryManager().destroyItem(tempInt);
		App.actPlayer.addCoins(500);
		App.actPlayer.cocoMilk = 2;
		CVM.stopConversation();
	}
	public void talk18() {
		int tempInt = App.actPlayer.getInventoryManager().searchItem("Coconut");
		...
		App.addInfo("Joe seems to get very angry...");
	...
	public void talk19() {
		...
		App.addInfo("Joe gives you 400 coins");
		App.actPlayer.getInventoryManager().destroyItem(App.actPlayer.getInventoryManager().searchItem("Coconut"));
		App.actPlayer.addCoins(400);
		App.actPlayer.cocoMilk = 2;
		CVM.stopConversation();
	}
	public void talk20() {
		App.addInfo("Joe hits you (You loose 20 HP), takes the coconut and throws 250 coins at you.");
		App.actPlayer.getInventoryManager().destroyItem(App.actPlayer.getInventoryManager().searchItem("Coconut"));
		App.actPlayer.addCoins(250);
		App.actPlayer.changeHP(-20);
		App.actPlayer.cocoMilk = 2;
		CVM.stopConversation();
	}
