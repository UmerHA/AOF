...
public class OptionPanel extends JPanel {
	...
	class actLis implements ActionListener {
		public void actionPerformed (ActionEvent e) {
			...
			if (e.getActionCommand().equalsIgnoreCase("Show Map [debug]")) 
				App.getGamePanel().shopScreen().setVisible(false);
			if (e.getActionCommand().equalsIgnoreCase("Show Shop [debug]")) 
				App.getGamePanel().shopScreen().setVisible(true);
			if (e.getActionCommand().equalsIgnoreCase("new trade invitation")) {
				App.getGamePanel().invitScreen().setName("The Option Panel");
	...
	class mLis extends MouseAdapter {
		public void mousePressed (MouseEvent e) {
			...
			if (source.equals("Show trade Panel [debug]")) {
				App.getGamePanel().invitScreen().setName("The OptionPanel", false);
				App.getGamePanel().tradeScreen().setVisible(true);
				...
				App.ANIManager().playAnimation(App.ANIManager().SWORD_CIRCLE, 200, 200);
			} else if (source.equals("Play Test Animation [Arrow]")) {
				System.out.println("OptionPanel$mLis :: sending animation request [Arrow]");
				App.ANIManager().playAnimation(App.ANIManager().ARROW, 200, 200);
			}
		}
	}
	...
	public void refresh () {
		...
		info[1].setText(" X : " + temp.getMapX() + " - " + App.map.getMapX() + " = " +(temp.getMapX()-App.map.getMapX()));
		info[1].setText(" Y : " + temp.getMapY() + " - " + App.map.getMapY() + " = " +(temp.getMapY()-App.map.getMapY()));
	}
