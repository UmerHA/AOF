...
import client.App;
import client.PanelWithBackPic;
import client.gamePanel.ChatBox;
import client.gamePanel.FieldPanel;
...
public class GamePanel_Old extends JPanel{
	...
	public GamePanel_Old () {
		...
		for (int i = 0; i < 14; i++)
			pics[i] = App.getImage(i + 1);
		bpak = App.getImage(16);

		//create sub-panels -->
		for (int i = 0; i < 14; i++)
			panels[i] = new PanelWithBackPic(pics[i]);
		panels[14] = new PanelWithBackPic(App.getImage(17));//invPanel test
		...
		for (int i = 0; i < 10; i++) {
			panels[i].setBounds(i * 50 + 1, App.HEIGHT()-45, 48, 40);
		}
		panels[10].setBounds(App.WIDTH()-100, App.HEIGHT()-50, 99, 50);
		panels[11].setBounds(App.WIDTH()-150, 0, 150, 125);
		panels[12].setBounds(App.WIDTH()-220, 0, 70, 40);
		panels[13].setBounds(App.WIDTH()-290, 0, 70, 40);
		y_bpak = App.HEIGHT()-50;
		...
		chatbox.setBounds(0, App.HEIGHT()-175, 400/*225*/, 125);
		add(chatbox);
		//add(menuPanel);
